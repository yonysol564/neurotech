...
******************************************************************************************/
	...
	function enqueue_my_styles() {
	    ...
	    wp_enqueue_style( 'normalize', $normalize, array(), 'v1', 'all' );
	    wp_enqueue_style( 'fonts', $fonts, array(), 'v1', 'all' ); 
...
******************************************************************************************/
	...
	function register_my_jscripts() {
	 ...
	 wp_register_script( 'foundation', THEME_DIR .'/js/foundation.min.js', array( 'jquery' ), '1', true ); wp_enqueue_script( 'foundation' );
		...
		wp_register_script( 'matchbox', THEME_DIR .'/js/matchbox.js', array( 'jquery' ), NULL, true ); wp_enqueue_script( 'matchbox' );
		wp_register_script( 'moment', THEME_DIR .'/js/moment.min.js', array( 'jquery' ), NULL, true ); wp_enqueue_script( 'moment' );
		//wp_register_script( 'slick', THEME_DIR .'/js/slick.js', array( 'jquery' ), NULL, true ); wp_enqueue_script( 'slick' );
...
function register_my_menu() {
    register_nav_menus(array(
		...
		'moxo_child_menu'   =>  'Moxo Child Menu'
    ));
}
	...
	$languages = icl_get_languages('skip_missing=0');
	  if(1 < count($languages)){
		    ...
		    foreach($languages as $l){
		      ...
		      $langs[] = '<li class="li_lang ' . $class . '"><a class="lang" title="'.$l['native_name'].'" href="'.$l['url'].'">'.  strtoupper($l['code']) .'</a></li>';
		    }
		    $langs[] = '</ul>';
	    echo join('', $langs);
	  }
